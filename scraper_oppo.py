...
async def select_os_brand(page: Page):
    """設定品牌為 OPPO，等到 select#phonename 有選項為止。"""
    for attempt in range(5):
        brand_value = await page.evaluate("""
            (() => {
                ...
                const oppoOpt = Array.from(brandSel.options).find(o =>
                    o.text.toUpperCase().includes('OPPO')
                );
                if (!oppoOpt) return null;
                brandSel.value = oppoOpt.value;
                brandSel.dispatchEvent(new Event('change', {bubbles: true}));
                return oppoOpt.value;
            })()
        """)
        ...
        """)
        if count > 1:
            print(f"  OPPO 品牌 value={brand_value}，找到 {count} 個機型")
            return
        await page.wait_for_timeout(1000)

    raise RuntimeError("select#phonename 無法載入 OPPO 機型，請檢查網站結構")


async def get_oppo_models(page: Page):
    ...
    print(f"找到 {len(models)} 個 OPPO 機型")
    return models
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print("取得 OPPO 機型清單...")
        models = await get_oppo_models(list_page)
        await list_ctx.close()

        if not models:
            print("ERROR: 找不到任何 OPPO 機型")
            await browser.close()
            return []
    ...
    with open("results_oppo.json", "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print(f"\n完成！共 {len(results)} 筆，已存入 results_oppo.json")
    return results
