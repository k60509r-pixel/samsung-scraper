...
def write_to_sheets(data: list[dict]):
    ...
    sheet = client.open_by_key(spreadsheet_id).worksheet("VIVO 回收報價")
    ...
    print(f"已寫入 {len(data)} 筆 VIVO 資料到 Google Sheets（價格補正欄已保留）")


if __name__ == "__main__":
    input_file = sys.argv[1] if len(sys.argv) > 1 else "results_vivo.json"
    try:
        with open(input_file, encoding="utf-8") as f:
            data = json.load(f)
    except FileNotFoundError:
        print(f"找不到 {input_file}，請先執行 scraper_vivo.py")
        sys.exit(1)
